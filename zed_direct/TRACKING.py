...
class handtracking():
    def __init__(self, max_hands = 2, detection_conf = 0.6, tracking_conf = 0.9) -> None:
        self.mp_hands = mp.solutions.hands
        self.model = mp.solutions.hands.Hands(max_num_hands = max_hands, min_detection_confidence = detection_conf, min_tracking_confidence = tracking_conf)
    ...
    def detect_landmarks(self, image):
        ...
        if self.results.multi_hand_landmarks:
            for landmarks in self.results.multi_hand_landmarks:
                self.draw.draw_landmarks(image, landmarks, self.mp_hands.HAND_CONNECTIONS)
